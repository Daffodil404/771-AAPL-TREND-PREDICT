"""Train and evaluate baseline methods (random, momentum, moving-average) for quantile labels."""
...
def train_random_baseline(test_df: pd.DataFrame, train_df: pd.DataFrame, seed: int = 42) -> pd.DataFrame:
    """Random baseline：按训练集类别比例随机抽样预测 test（基于 quantile 标签）。"""
    rng = np.random.default_rng(seed)
    probs = (
        train_df[TARGET_COLUMN]
        .value_counts(normalize=True)
        .reindex(["up", "down", "flat"], fill_value=0)
    )
...
def train_momentum_baseline(test_df: pd.DataFrame) -> pd.DataFrame:
    """Momentum baseline：用当日 daily_return_rate 的符号/阈值预测下一日方向（quantile 标签版本）。"""
...
def train_moving_average_baseline(
    ...
    """Moving-average baseline：用 close 的 MA5 vs MA20 判断 up/down/flat（quantile 标签版本）。"""
...
def main() -> None:
    ...
    # MA baseline：需要 close，从 processed 读
    ...
    print("Quantile baselines training done.")
