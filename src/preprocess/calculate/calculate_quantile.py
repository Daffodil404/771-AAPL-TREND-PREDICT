...
def output_data(df: pd.DataFrame, output_path: Path) -> None:
    ...
    out["target_label_next_day_quantile"] = calculate_target_next_day_label_quantile(out)
    ...
    out["target_label_next_3d_quantile"] = calculate_target_next_3d_label_quantile(out)

    # Keep a separate quantile label for compatibility in quantile pipeline.
    out["label_quantile"] = out["target_label_next_day_quantile"]
...
def main() -> None:
    input_path = PROJECT_ROOT / "data" / "processed" / "aapl_clean.csv"
